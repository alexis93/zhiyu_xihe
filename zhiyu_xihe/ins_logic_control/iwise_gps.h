 ...
 *
 * 	version :
 * 	history	: 	2015-06-25 created by chenxi
 *-------------------------------------------------------------------*/
#ifndef _IWISE_GPS_H
#define _IWISE_GPS_H
...
#define GPS_POSITION_MODE_XIHE			6
#define GPS_POSITION_MODE_DP			5
 ...
 * 添加
 * */
typedef void (* gps_rinex_callback)(GpsUtcTime timestamp, const char* rinex, int length);
/** Callback to inform framework of the GPS engine's capabilities.
 *  Capability parameter is a bit field of GPS_CAPABILITY_* flags.
 */
 
typedef void(* gps_message_callback)(iwise_message_t* message);

typedef void(* gps_sol_callback)(sol_t* sol);
...
typedef struct{
	...
	void (*set_main_server)(const char* hostname, int port);
	
	void (*set_extension_server)(const char* hostname, int port);
	
	void (*inject_extern_data)(int data_type, char* data, int size);
