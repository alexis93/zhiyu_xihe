...
#define MATCH(s, n) strcmp(section, s) == 0 && strcmp(name, n) == 0
	...
	if (MATCH ("nmea_server","nmea_server_rcvaddress")){
		pconfig->nmea_server_address = strdup(value);
	} else if(MATCH ("nmea_server","nmea_server_rcvport")){
		pconfig->nmea_server_port = (uint16_t) atoi(value);
	} else if(MATCH ("gnss_server", "gnss_server_rcvaddress")) {
		pconfig->gnss_server_address = strdup(value);
	} else if (MATCH("gnss_server", "gnss_server_rcvport")) {
		pconfig->gnss_server_port = (uint16_t) atoi(value);
	}  else if(MATCH ("gnss_nmea_server", "gnss_nmea_server_rcvaddress")) {
		pconfig->gnss_nmea_server_address = strdup(value);
	} else if (MATCH("gnss_nmea_server", "gnss_nmea_server_rcvport")) {
		pconfig->gnss_nmea_server_port = (uint16_t) atoi(value);
	} else if (MATCH("ins_server","ins_server_rcvaddress")){
		pconfig->ins_server_address = strdup(value);
	} else if (MATCH("ins_server","ins_server_rcvport")){
		pconfig->ins_server_port = (uint16_t)atoi(value);
	} else if (MATCH("ins_nmea_server","ins_nmea_server_rcvaddress")){
		pconfig->ins_nmea_server_address = strdup(value);
	} else if (MATCH("ins_nmea_server","ins_nmea_server_rcvport")){
		pconfig->ins_nmea_server_port = (uint16_t)atoi(value);
	} else if (MATCH("log_tcpserver","log_tcpserver_rcvaddress")){
		pconfig->log_tcpserver_address = strdup(value);
	} else if (MATCH("log_tcpserver","log_tcpserver_rcvport")){
		pconfig->log_tcpserver_port = (uint16_t)atoi(value);
	} else if (MATCH("obs_log_tcpserver","obs_log_tcpserver_rcvaddress")){
		pconfig->obs_log_tcpserver_address = strdup(value);
	} else if (MATCH("obs_log_tcpserver","obs_log_tcpserver_rcvport")){
		pconfig->obs_log_tcpserver_port = (uint16_t)atoi(value);
	} else if (MATCH("eph_log_tcpserver","eph_log_tcpserver_rcvaddress")){
		pconfig->eph_log_tcpserver_address = strdup(value);
	} else if (MATCH("eph_log_tcpserver","eph_log_tcpserver_rcvport")){
		pconfig->eph_log_tcpserver_port = (uint16_t)atoi(value);
	} else if (MATCH("clockorbit_log_tcpserver","clockorbit_log_tcpserver_rcvaddress")){
		pconfig->clockorbit_log_tcpserver_address = strdup(value);
	} else if (MATCH("clockorbit_log_tcpserver","clockorbit_log_tcpserver_rcvport")){
		pconfig->clockorbit_log_tcpserver_port = (uint16_t)atoi(value);
	} else if (MATCH("msg_log_tcpserver","msg_log_tcpserver_rcvaddress")){
		pconfig->msg_log_tcpserver_address = strdup(value);
	} else if (MATCH("msg_log_tcpserver","msg_log_tcpserver_rcvport")){
