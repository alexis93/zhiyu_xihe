 ...
 *
 *  version :
 *  history	: 2015-03-18 created by lzg
 *-------------------------------------------------------------------*/
...
typedef struct {        /* SBAS(地区性广域差分增强系统) ephemeris type */
    int sat;            /* satellite number */
...
typedef struct {        /* TEC(电离层) grid type */
...
typedef struct {        /* stec(GPS 信号传播路径上的电子总含量) data type */
    gtime_t time;       /* time (GPST) */
