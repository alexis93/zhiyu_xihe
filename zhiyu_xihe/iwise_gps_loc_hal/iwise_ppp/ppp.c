...
static void udtrop_ppp(rtk_t *rtk)
{
    ...
    if (rtk->x[i]==0.0) {
        ...
        rtk->P[i*(1+rtk->nx)]+=SQR(rtk->opt.prn[2])*fabs(rtk->tt);   //���Ϲ�������;
        
        if (rtk->opt.tropopt>=TROPOPT_ESTG) {
            for (j=0;j<2;j++) {
				rtk->P[++i*(1 + rtk->nx)] += SQR(rtk->opt.prn[2] * 0.1)*fabs(rtk->tt) * 1000;
            }
        }
    }
}
...
//added by yuan;
static void udion_ppp(rtk_t *rtk, obsd_t *obs, int n,nav_t *nav)
{
	double pos[3]={0}, azel[] = { 0.0, PI / 2.0 }; double ion = 0;
	int j = 0; int sat = 0;
	double var=0; int k = 0;
	...
	for (j = 0; j < n&&j < MAXOBS; j++) {
		...
		if (rtk->x[II(sat, &rtk->opt)] == 0.0) {
			ecef2pos(rtk->sol.rr, pos);
			//i = II(sat, &rtk->opt);
			ionocorr(obs->time, nav, obs->sat, pos, azel,
					IONOOPT_BRDC, &ion, &var);
			//if (!corrmeas(obs + j, nav, pos, rtk->ssat[sat - 1].azel, &rtk->opt, NULL, NULL,
			//	0.0, meas, var, NULL)) continue;
			initx(rtk, ion, ERR_BRDCI, II(sat, &rtk->opt));    //;
		}
		else{
			rtk->P[k*(1 + rtk->nx)] += SQR(rtk->opt.prn[1])*fabs(rtk->tt)*1000;   //;
		}
	}
}
...
//added by yuan
extern int detect_ppp(rtk_t *rtk, obsd_t *obs, int n, nav_t *nav)
{
	double offset = 0.0;
	int i = 0, j = 0, k = 0, sat = 0;
	int Js = 0;
...
static void udbias_ppp(rtk_t *rtk,obsd_t *obs, int n, nav_t *nav)
{
    double meas[4],var[2],bias[MAXOBS]={0},pos[3]={0};
	int i = 0, j = 0, k = 0, sat = 0, brk = 0;
	//int mark = 0;   //;
...
static void udstate_ppp(rtk_t *rtk, obsd_t *obs, int n, nav_t *nav)
{
                   ...
                   double *H, double *R, double *azel)
{
    ...
    double r,rr[3],pos[3],e[3],meas[4],dtdx[3],dantr[NFREQ]={0};
    double dants[NFREQ]={0},var[MAXOBS*2],dtrp=0.0,vart=0.0,varm[2]={0};
    int i,j,k,sat,sys,nv=0,nx=rtk->nx,brk;

	double z = 0; double F_z = 0; int sat1 = 0; int sat2 = 0;
...
extern void pppos(obsd_t *obs, int n, nav_t *nav, rtk_t *rtk)
{
	...
	int kk = 0; double temp = 0;
	double rs[6 * MAXOBS_];
	double dts[2 * MAXOBS_];
	double var[1 * MAXOBS_];
	double azel[2 * MAXOBS_];
	double xp[1 * 135];
	double Pp[135 * 135];
	double v[MAXOBS_ * 1 * 3];
	double H[MAXOBS_ * 135];
	double R[MAXOBS_ * MAXOBS_];

	for (i=0;i<MAXSAT;i++) rtk->ssat[i].fix[0]=0;
//
	initmat(rs, MAXOBS_, 6);
	initmat(dts, MAXOBS_, 2);
	initmat(var, MAXOBS_, 1);
	initmat(azel, MAXOBS_, 2);
	initmat(xp, 135, 1);
	initmat(Pp, 135, 135);
	initmat(v, MAXOBS_, 3);
	initmat(H, MAXOBS_, 135);
	initmat(R, MAXOBS_, MAXOBS_);
	...
	matcpy(xp, rtk->x, rtk->nx, 1); nv = n*opt->nf * 3; //p = n*opt->nf * 3;    //�ز���α��;
	/* measurement update */
...
extern int ppp_pos_sol(obsd_t *obs,int n , nav_t *nav, char *msg, rtk_t *rtk)
{
	...
	if (!pntpos(rtk, obs_t, n,nav,aze, msg)) {
		return 0;
	}
	printf("pntpos\n");
	rtk->sol.nav = 1;  //SPP;
