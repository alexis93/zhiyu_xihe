...
#define MAXSOLMSG   8192            /* max length of solution message */
#define MAXRAWLEN   4096                /* max length of receiver raw message */
#define MAXERRMSG   4096                /* max length of error/warning message */
#define MAXANT      64                  /* max length of station name/antenna type */
#define MAXSOLBUF   256                 /* max number of soint last_error;*/ 
...
#define IONOOPT_TEC 5                   /* ionosphere */
...
#define STRFMT_NEWSTAR 19
#define STRFMT_HEXIN 50        //zhuyong add 2015.7.17,hexinxintong
#ifndef EXTLEX
...
#define ENMEA_GSV_MSG_MISSING  0X1001
#define ENMEA_GGA_MSG_MISSING  0X1002
#define ENMEA_GSV_MSG_PARSE    0X1003
#define ENMEA_GGA_MSG_PARSE    0X1004
#define ENMEA_CHECKSUM         0X1005
#define ENMEA_EXTRACT_FIELD    0X1006          
#define ENMEA_HEAD             0X1007
#define ENMEA_GGA_INVALID      0X1008
#define ENMEA_CALLOC_FAILED    0X1009
#define ENMEA_UNKNOWN		    0X1010
#define ERAW_UBX_LENGTH_ERROR		0X1011
#define ERAW_UBX_CHECKSUM_ERROR    0X1012
#define ERAW_UBX_RXMRAW_LENGTH_ERROR	0X1013
...
#define G_DEBUG 2
#if G_DEBUG
#define GLOG(fmt, args...) ((int)_gnss_log_file("%s; %s; %04d; "fmt"\n",__FILE__,__FUNCTION__,  __LINE__,  ##args));
#else
#define GLOG(fmt, args...) ((void)0)
#endif
...
typedef struct {        /* norad(北美防空司令部) two line element data type */
...
typedef struct {        /* zwd(对流层天顶湿延迟) data type */
...
extern void degtodms(double deg, double *dms);
...
/*gnss_log functions ---------------------------------------------------------*/
extern int _gnss_log_file(char *fmt, ...);
...
extern int outrnxobsh_s(char *buff, const rnxopt_t *opt, const nav_t *nav);
extern int outrnxobsb_s(char *buff, const rnxopt_t *opt, const obsd_t *obs, int n,int flag);
extern int outrnxnavh_s(char *buff, const rnxopt_t *opt, const nav_t *nav);
extern int outrnxnavb_s(char *buff, const rnxopt_t *opt, const eph_t *eph);   
...
extern int input_hexin (raw_t *raw, unsigned char data);  //zhuyong add 2015.7.17
...
extern int input_newstar(raw_t *raw, unsigned char data);
...
//TODO lzg
/* alter positioning ---------------------------------------------------------*/
extern void getdeltaposs(const obsd_t *obs, int n, const nav_t *nav, struct ClockOrbit *correct,int ephopt, double * delta);
extern void correctposs(const double * delta, const int n, double *rs, double * dts);
extern void satposs_delta(gtime_t teph, const obsd_t *obs, int n, const nav_t *nav,
                    int ephopt, double *rs, double *dts, double *var, int *svh);
/*extern int pntpos_ex(const obsd_t *obs, int n, const nav_t *nav, const prcopt_t *opt,
	 double *rs, double *dts, double *var, int *svh, sol_t *sol, double *azel, ssat_t *ssat, char *msg);*/
/*extern int pnt_pos_sol(const obs_t *obs, const nav_t *nav, const prcopt_t *opt, const clockorbit_t * clockorbit,
				sol_t *sol,  ssat_t *ssat, char *msg);*/
/*extern int pnt_pos_xyz(const obs_t *obs, const nav_t *nav, const prcopt_t *opt, const clockorbit_t * clockorbit,
                    double *x, double *y, double *z,  ssat_t *ssat, char *msg);
extern int pnt_pos_lon_lat(const obs_t *obs, const nav_t *nav, const prcopt_t *opt, const clockorbit_t * clockorbit,
                    double *lon, double *lat,  ssat_t *ssat, char *msg);
extern int pnt_pos_nmea(const obs_t *obs, const nav_t *nav, const prcopt_t *opt, const clockorbit_t * clockorbit,
                    unsigned char *nmea,  ssat_t *ssat, char *msg);


extern void xyz2lon_lat(const double *x, const double *y, const double *z,
				double *lon, double *lat);
extern void lon_lat2xyz(const double *lon, const double *lat,
				double *x, double *y, double *z);
extern double distance(const double *lon1, const double *lat1, const double *lon2, const double *lat2);*/
/* precise positioning -------------------------------------------------------*/
...
/* alter point positioning ---------------------------------------------------*/
/*extern void pppos_ex(rtk_t *rtk, const obsd_t *obs, int n, const nav_t *nav,double *rs,double *dts,double *var,int *svh);
extern int ppp_pos_sol(rtk_t *rtk, const obs_t *obs, const nav_t *nav, const clockorbit_t * clockorbit);
extern int ppp_pos_xyz(rtk_t *rtk, const obs_t *obs, const nav_t *nav, const clockorbit_t * clockorbit,
                    double *x, double *y, double *z);
extern int ppp_pos_lon_lat(rtk_t *rtk, const obs_t *obs, const nav_t *nav, const clockorbit_t * clockorbit,
                    double *lon, double *lat);
extern int ppp_pos_nmea(rtk_t *rtk, const obs_t *obs, const nav_t *nav, const clockorbit_t * clockorbit,
                    unsigned char *nmea);*/
